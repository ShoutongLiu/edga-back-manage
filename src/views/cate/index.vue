...
import { addCategroy, getCategroy, delCategroy, updateCategroy } from '@/api/categroy'
export default {
    data () {
        return {
            cateInfo: {
    ...
    methods: {
        ...
        // 删除一条数据
        handelDel (row) {
            delCategroy({ _id: row._id }).then(res => {
                if (res.data.isDel) {
        ...
        confirmUpdate () {
            ...
            updateCategroy(this.row).then(res => {
